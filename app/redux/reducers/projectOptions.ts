...
const projectOptions = createSlice({
  ...
  reducers: {
    ...
    setIsBuilding: (state, action: PayloadAction<{ isbuilding: boolean }>) => {
      return {
        ...state,
    ...
    setProjectMode: (
      state,
      action: PayloadAction<{ mode: "preview" | "dashboard" | "code" }>
    ) => {
      return {
        ...state,
        mode: action.payload.mode,
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (
        ...
        state.isbuilding = action.payload.isbuilding;
      }
    );
