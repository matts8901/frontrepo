...
function extractMessage(input: string) {
...
export const useGenerateFile = () => {
  ...
  const genFile = async ({ email, projectId, input }: GenerateFileParams) => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API}/agent`, {
        method: "POST",
        headers: {
  ...
  const sendMessagetoAI = async ({
    ...
    try {
      ...
      if (files) {
        ...
        try {
          mes = extractMessage(completeContent);
          handleMessage(mes || "task completed");
        } catch (error) {
          console.log("failed to extract message - ", error);
        }
      } else {
        dispatch(
          setGenerating({
