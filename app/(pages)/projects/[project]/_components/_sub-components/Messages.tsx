...
const Messages: NextPage = () => {
  ...
  return (
    <div className="flex flex-col gap-5 p-4 overflow-y-auto flex-1 z-30">
      {messages &&
        messages.length > 0 &&
        messages.map((msg, index) => {
          ...
          return (
            <div
              key={messageKey}
              className={`z-40 flex flex-col ${msg.role === "user" ? "items-end px-1" : "items-start"}`}
            >
              ...
              {messages[index + 1] && messages[index + 1].role !== msg.role && (
                <motion.div
                  className="w-full h-[1px] bg-gradient-to-r bg-[#0A0A0A] via-gray-700 to-[#0A0A0A] opacity-50 my-3"
